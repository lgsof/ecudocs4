	...
	def __init__ (self, *args, **kwargs):
		...
		self.base_columns['acciones'] = tables.TemplateColumn(
			...
			<div class="btn-group btn-group-sm" role="group">
			  <a href="{{ record.get_link_pdf }}?pk={{ record.id }}&pdfType=original"
				 target="_blank" class="btn btn-outline-dark btn-sm p-1 lh-1"
				 data-bs-toggle="tooltip" title="PDF Original">
				<i class="bi bi-file-earmark-pdf fs-6 pdf-original"></i>
			  </a>

			  <a href="{{ record.get_link_pdf }}?pk={{ record.id }}&pdfType=copia"
				 target="_blank" class="btn btn-outline-dark btn-sm p-1 lh-1"
				 data-bs-toggle="tooltip" title="PDF Copia">
				<i class="bi bi-file-earmark-pdf fs-6 pdf-copia"></i>
			  </a>

			  <a href="{{ record.get_link_pdf }}?pk={{ record.id }}&pdfType=paquete"
				 target="_blank" class="btn btn-outline-dark btn-sm p-1 lh-1"
				 data-bs-toggle="tooltip" title="PDF Paquete">
				<i class="bi bi-file-earmark-zip fs-6 pdf-paquete"></i>
			  </a>

			  <form method="post" action="{{ record.get_link_eliminar }}" class="d-inline"
					onsubmit="return confirm('¿Eliminar este documento?');">
				{% csrf_token %}
				<button type="submit"
						class="btn btn-outline-danger btn-sm p-1 lh-1 delete-btn"
			  ...
			  <a href="{{ record.get_link_detalle }}"
				 class="btn btn-info btn-sm p-1 lh-1 open-doc" data-win="doc_{{ record.pk }}"
				 data-bs-toggle="tooltip" title="Detalle" aria-label="Detalle">
...
#			<div class="btn-group btn-group-sm" role="group">
#				<a href="{{ record.get_link_pdf }}?pk={{ record.id }}&pdfType=original"
#				   target="_blank" class="btn btn-outline-dark" data-bs-toggle="tooltip" title="PDF Original">
#					<i class="bi bi-file-earmark-pdf pdf-icon pdf-original"></i>
#				</a>
#
#				<a href="{{ record.get_link_pdf }}?pk={{ record.id }}&pdfType=copia"
#				   target="_blank" class="btn btn-outline-dark" data-bs-toggle="tooltip" title="PDF Copia">
#					<i class="bi bi-file-earmark-pdf pdf-icon pdf-copia"></i>
#				</a>
#
#				<a href="{{ record.get_link_pdf }}?pk={{ record.id }}&pdfType=paquete"
#				   target="_blank" class="btn btn-outline-dark" data-bs-toggle="tooltip" title="PDF Paquete">
#					<i class="bi bi-file-earmark-zip pdf-icon pdf-paquete"></i>
#				</a>
#
#				<a href="{{ record.get_link_eliminar }}"
#				   class="btn btn-danger open-doc" data-win="doc_{{ record.pk }}"
#				   data-bs-toggle="tooltip" title="Eliminar">
#					<i class="bi bi-trash delete-icon"></i>
#				</a>
#
#				<a href="{{ record.get_link_detalle }}"
#				   class="btn btn-info open-doc" data-win="doc_{{ record.pk }}"
#				   data-bs-toggle="tooltip" title="Detalle">
#					<i class="bi bi-card-list detail-icon"></i>
#				</a>
#			</div>
#			''',
#		)
...
#		return format_html(
#			'''
#			<a href="{}" class="btn btn-sm btn-danger" title="PdfO">
#				<i class="bi bi-trash"></i>
#			</a>
#			''',
#			reverse("cartaporte-pdf", args=[record.id]),
#		)
