...
class CartaporteInfo (EcuInfo):
	...
	def extractEcuapassFields (self, docFieldsPath, analysisType="BOT"):
		...
		Utils.debug (remitente, "remitente:")
	...
	def getMercanciaInfoCartaporte (self, analysisType="BOT"):
		ecuapassFields = {"cantidad":"10_CantidadClase_Bultos", "marcas":"11_MarcasNumeros_Bultos", 
					"descripcion":"12_Descripcion_Bultos", 'pesoNeto':'13a_Peso_Neto', 'embalaje':'11_MarcasNumeros_Bultos',
	...
	def getGastosInfo (self):
		...
		try:
			# DESTINATARIO:
			gastos ["fleteDest"]	   = self.getEcuapassValueFromField ("17_Gastos:ValorFlete,MontoDestinatario")
			gastos ["seguroDest"]      = self.getEcuapassValueFromField ("17_Gastos:Seguro,MontoDestinatario")
			gastos ["otrosDest"]       = self.getEcuapassValueFromField ("17_Gastos:OtrosGastos,MontoDestinatario")
			gastos ["totalDest"]       = self.getEcuapassValueFromField ("17_Gastos:Total,MontoDestinatario")

			# REMITENTE: 
			gastos ["fleteRemi"]       = self.getEcuapassValueFromField ("17_Gastos:ValorFlete,MontoRemitente")
			gastos ["seguroRemi"]      = self.getEcuapassValueFromField ("17_Gastos:Seguro,MontoRemitente")
			gastos ["otrosRemi"]       = self.getEcuapassValueFromField ("17_Gastos:OtrosGastos,MontoRemitente")
			gastos ["totalRemi"]       = self.getEcuapassValueFromField ("17_Gastos:Total,MontoRemitente")

			gastos ["monedaDest"]      = Utils.getMonedaUSD (gastos ["fleteDest"])
			gastos ["otrosDestMoneda"] = Utils.getMonedaUSD (gastos ["otrosDest"])
			gastos ["totalDestMoneda"] = Utils.getMonedaUSD (gastos ["totalDest"])
			gastos ["monedaRemi"]      = Utils.getMonedaUSD (gastos ["fleteRemi"])
			gastos ["otrosRemiMoneda"] = Utils.getMonedaUSD (gastos ["otrosRemi"])
			gastos ["totalRemiMoneda"] = Utils.getMonedaUSD (gastos ["totalRemi"])
	...
	def totalizeCheckGastos (self, gastos):
		#---------------------------------------------------------------
		def FLOAT (value):
			return float (Extractor.getNumberUSFormat (value)) if value else 0.0
		#---------------------------------------------------------------
		def totalizeOtros (keys):
			...
			try:
				if gastos [keys [1]] or gastos [keys [2]]:
					...
					gastos [monedaKey] = "USD" if gastos [totalKey] else None
			except:
		...
		Utils.debug (f"\n+++ Gastos Info: '{gastos}'", "totalizeCheckGastos")
	...
	def getSubjectInfo (self, key, text=None):
		...
		try:
			...
			print (f"\t+++ SubjectInfo for Nombre-Dire: Removed NombreDireccion '{subject}'\n'{text}'")
		except:
	...
	def processIdTypeNum (self, subject):
		...
		try:
			if tipoId == "NIT":
				subject ['tipoId']   = Utils.addWarning ("OTROS", "Se cambio OTROS en vez de NIT")
				subject ['numeroId'] = Utils.addWarning (numeroId.split ("-")[0], "Se eliminó último dígito del NIT (XXXXXXX-Y)") 
		except:
