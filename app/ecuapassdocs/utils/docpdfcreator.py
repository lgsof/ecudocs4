...
# For creating Http response
from django.http import HttpResponse

# App
import app_cartaporte, app_manifiesto
from ecuapassdocs.utils.resourceloader import ResourceLoader 
from ecuapassdocs.info.ecuapass_utils import Utils 

print (f"+++ ...Loading DocPdfCreator module")
#----------------------------------------------------------------
# Crea un documento PDF con background y textos
#----------------------------------------------------------------
class DocPdfCreator:
	def __init__ (self, docType, pdfType="ONE_PDF"):
	...
	#-------------------------------------------------------------------
	# Functions for creating Http PDF response 
	#-------------------------------------------------------------------
	def createPdf (self, pdfType, formFields):
	...
	#-- Create a single PDF without related documents 
	def createPdfResponseSingleDoc (self, formFields, pdfType):
		try:
			print ("+++ Creando respuesta PDF simple...")
			creadorPDF = DocPdfCreator (self.docType, "ONE_PDF")
			print (f"\n+++ '{self.docType=}'")
			print (f"\n+++ '{pdfType=}'")
	...
	def createPdfResponseMultiDoc (self, docFields):
		try:
			...
			creadorPDF = DocPdfCreator ("MULTI_PDF")
	...
	def getInputValuesForDocumentChilds (self, docType, docId):
		...
		try:
			...
			Utils.printException (f"Error obteniendo documentos asociados a '{docType}'::id={docId}")
	...
	def createPdfFileMultiDoc (self, values, types):
		...
		outPdfPath  = os.path.join (tmpPath, f"{self.prefix}-{docNumber}.pdf") 
	...
	def createPdfFileSingleDoc (self, docType, formFields, pdfType):
		...
		copyStringDic = {"CARTAPORTE":"txt24", "MANIFIESTO":"txt41", "DECLARACION":"txt26"}
		formFields [copyStringDic [docType]] = "ORIGINAL" if "original" in pdfType.lower() else "COPIA"

		tmpPath     = tempfile.gettempdir ()
		outPdfPath  = os.path.join (tmpPath, f"{self.prefix}-{docNumber}.pdf") 
		outJsonPath = os.path.join (tmpPath, f"{self.prefix}-{docNumber}.json") 
	...
	def writeInputsToPdf (self, docType, paramFields, formFields):
		...
		for key, params in paramFields.items():
			if not key.startswith ("txt") or "NO_FORMFIELD" in params ["class"]:
				continue
	...
	def embedFieldsIntoPDFCartaporte (self, pdfCanvas, paramFields, formFields):
		...
		for key, params in paramFields.items():
			...
			if "Gastos" in fieldName:
				res = re.findall (r"\w+", fieldName)   #e.g ["ValorFlete", "MontoDestinatario"]
...
if __name__ == "__main__":
    ...
    creadorPDF = DocPdfCreator ("CARTAPORTE")
