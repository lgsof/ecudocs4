...
#		return (value_strnumber)
	...
	#-- If None return "||LOW"
	def checkLow (data, message=None):
		message = f"||LOW:{message}" if message else "||LOW:No existe el dato"
		if type (data) == dict:
			for k in data.keys ():
				data [k] = data [k] if data [k] else message
		else:
